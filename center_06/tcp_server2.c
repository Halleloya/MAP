	...
	int QoS_min[CLIENT_NUM] = {2, 2, 2, 2}; //各用户的最小freq要求
    ...
    {  
        ...
        {  
			...
			*/
            // fputs(buffer, stdout);
			/*
			for (p=0; p<CLIENT_NUM-1; p++)
			{
				if (client_flag[p] == client_flag[p+1])
				{
					printf("\nclient_flag[p]:%d \n", client_flag[p]);
					id = p;
					client_flag[id] ++;
			...
			printf("\npid:%d && id:%d receive:\n",pid, id);  
            fputs(buffer, stdout);
			cpu_power[id] = atof(buffer);
			printf("\ncpu_power[id]:%lf\n",cpu_power[id]);			
			change_id();
				
			
			if(client_flag[0]==client_flag[1]&&client_flag[0]==client_flag[2]&&client_flag[0]==client_flag[3])
			{
				if (q == 0)
					q ++;
				else if (q>0 && q<5)
