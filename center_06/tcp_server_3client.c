	...
	#define SUPPLY 39  //37-povray 30-stream ebizzy
	#define CLIENT_NUM 3
	#define INIT_P0 1000    
	...
	double cpu_power_default[CLIENT_NUM] = {20, 21.5, 10};
	double cpu_power_expect[CLIENT_NUM] = {0, 0, 0};
	...
	int QoS_min[CLIENT_NUM] = {5, 5, 12}; //各用户的最小freq要求
	int demand[CLIENT_NUM] = {12, 12, 12}; //各用户的当前freq要求，最后发给各方的就是freq_level[demand[j]]
	int demand_last[CLIENT_NUM] = {12, 12, 12};
	double sum_d = 0;
	int num_fit[CLIENT_NUM] = {0, 0, 0};
	...
	void init_file() //delete the history of previous results
	{
		...
		fprintf(fp_init, "%d\t%d\t%d", 12, 12, 12);
		fclose(fp_init);
	...
	void final_price() //calculate the final price
	{
		...
		printf("price_var %d \n", price_var);
		printf("num_fit %d, %d, %d \n", num_fit[0], num_fit[1], num_fit[2]);
